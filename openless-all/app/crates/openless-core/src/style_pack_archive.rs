...
use crate::style_packs::StylePackExample;
use crate::types::PolishMode;

fn ensure_dir(path: &Path) -> Result<()> {
    fs::create_dir_all(path)
        .with_context(|| format!("create style pack asset directory: {}", path.display()))
}

pub const STYLE_PACK_ARCHIVE_MAX_COMPRESSED_BYTES: usize = 512 * 1024;
...
pub fn validate_style_pack_archive_bytes(compressed: &[u8]) -> Result<()> {
