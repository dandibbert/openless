    ...
    while cursor < edit_plan_inner.len() {
        ...
        for tag in EDIT_OPERATION_TAGS {
            if let Some(pos) = find_open_tag(edit_plan_inner, tag, cursor) {
                if next.is_none_or(|(best, _)| pos < best) {
                    next = Some((pos, tag));
                }
            }
        }
...
fn parse_operation_xml(tag: &str, inner: &str, opening_tag: &str) -> Result<EditOperation, String> {
    match tag {
        ...
        "range_replace" => {
            let start = parse_u32_attr(opening_tag, "start")
                .or_else(|| {
                    extract_child_text(inner, "start").and_then(|text| parse_u32_text(&text))
                })
                .unwrap_or(0);
...
fn extract_element_block(
    ...
    let start = find_open_tag(content, tag, from).ok_or_else(|| format!("<{tag}> not found"))?;
...
fn validate_operation_size(op: &EditOperation) -> Result<(), EditApplyError> {
    ...
    match op {
        ...
        EditOperation::RegexReplace {
            ...
            if pattern.chars().count() > MAX_PATTERN_LEN || too_large(replace) {
