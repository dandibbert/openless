...
constexpr wchar_t kClsidKey[] = L"Software\\Classes\\CLSID\\{6B9F3F4F-5EE7-42D6-9C61-9F80B03A5D7D}";
...
HRESULT SetStringValue(HKEY key, const wchar_t *name, const wchar_t *value) {
  const auto byte_count = static_cast<DWORD>((wcslen(value) + 1) * sizeof(wchar_t));
  return HResultFromWin32Error(
      RegSetValueExW(key, name, 0, REG_SZ, reinterpret_cast<const BYTE *>(value), byte_count));
}

HRESULT RegisterComServer(HINSTANCE module) {
  wchar_t module_path[MAX_PATH] = {};
  const DWORD path_len = GetModuleFileNameW(module, module_path, ARRAYSIZE(module_path));
  if (path_len == 0) {
  ...
  LSTATUS status =
      RegCreateKeyExW(HKEY_LOCAL_MACHINE, kClsidKey, 0, nullptr, REG_OPTION_NON_VOLATILE,
                      kRegistryWriteAccess, nullptr, &clsid_key, nullptr);
  ...
  status =
      RegCreateKeyExW(HKEY_LOCAL_MACHINE, kInprocServer32Key, 0, nullptr, REG_OPTION_NON_VOLATILE,
                      kRegistryWriteAccess, nullptr, &inproc_key, nullptr);
...
public:
  ...
  HRESULT hr() const { return hr_ == RPC_E_CHANGED_MODE ? S_OK : hr_; }
...
HRESULT CreateCategoryManager(ITfCategoryMgr **category_mgr) {
  return CoCreateInstance(CLSID_TF_CategoryMgr, nullptr, CLSCTX_INPROC_SERVER, IID_ITfCategoryMgr,
...
HRESULT RegisterLanguageProfile() {
  ...
  if (SUCCEEDED(hr)) {
    hr = profiles->AddLanguageProfile(CLSID_OpenLessTextService, kOpenLessLangId,
                                      GUID_OpenLessProfile, const_cast<wchar_t *>(kOpenLessImeName),
                                      static_cast<ULONG>(ARRAYSIZE(kOpenLessImeName) - 1), nullptr,
                                      0, 0);
  }
  if (SUCCEEDED(hr)) {
    hr = profiles->EnableLanguageProfile(CLSID_OpenLessTextService, kOpenLessLangId,
                                         GUID_OpenLessProfile, TRUE);
  }
  ...
  hr = manager->RegisterProfile(
      ...
      static_cast<ULONG>(ARRAYSIZE(kOpenLessImeName) - 1), nullptr, 0, 0, nullptr, 0, TRUE,
...
HRESULT RegisterKeyboardCategory() {
  ...
  if (SUCCEEDED(hr)) {
    hr = category_mgr->RegisterCategory(
        CLSID_OpenLessTextService, GUID_TFCAT_TIPCAP_IMMERSIVESUPPORT, CLSID_OpenLessTextService);
  }
  if (SUCCEEDED(hr)) {
    hr = category_mgr->RegisterCategory(CLSID_OpenLessTextService, GUID_TFCAT_TIPCAP_SYSTRAYSUPPORT,
                                        CLSID_OpenLessTextService);
  }
...
HRESULT UnregisterKeyboardCategory() {
  ...
  if (SUCCEEDED(hr)) {
    hr = category_mgr->UnregisterCategory(
        CLSID_OpenLessTextService, GUID_TFCAT_TIPCAP_SYSTRAYSUPPORT, CLSID_OpenLessTextService);
  }
  category_mgr->Release();
  return hr;
}
