...
export function saveStylePack(stylePack: StylePack): Promise<StylePack> {
  return invokeOrMock('save_style_pack', { stylePack }, () => mockSaveStylePack(stylePack));
}

export function readStylePackIcon(id: string): Promise<string | null> {
  return invokeOrMock('read_style_pack_icon', { id }, () =>
    localStorage.getItem(`ol.stylePackIcon.${id}`),
  );
}
...
export function setStylePackIcon(id: string, png: number[] | null): Promise<StylePack> {
  return invokeOrMock('set_style_pack_icon', { id, png }, () => {
    ...
    return mockSaveStylePack({
      ...pack,
      iconPath: png ? `preview:${id}` : null,
...
export function setActiveStylePack(id: string): Promise<StylePack> {
  return invokeOrMock('set_active_style_pack', { id }, () => mockSetActiveStylePack(id));
}
...
export function resetBuiltinStylePack(id: string): Promise<StylePack> {
  return invokeOrMock('reset_builtin_style_pack', { id }, () => mockResetBuiltinStylePack(id));
}
