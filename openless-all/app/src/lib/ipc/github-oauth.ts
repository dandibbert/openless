...
export function githubDeviceFlowPoll(flowId: string): Promise<GithubDevicePollResult> {
  return invokeOrMock<GithubDevicePollResult>('github_device_flow_poll', { flowId }, () => ({
...
export function marketplaceAuthStatus(): Promise<MarketplaceAuthStatus> {
  return invokeOrMock<MarketplaceAuthStatus>('marketplace_auth_status', undefined, () => ({
    signedIn: false,
  }));
}
